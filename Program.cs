...
app.MapGet("/administradores", ([FromQuery] int? pagina, IAdministradorServico administradorServico) => {
    ...
    var adiministradores = administradorServico.Todos(pagina);

    foreach (var adm in adiministradores) {
...
RequireAuthorization(new AuthorizeAtribbute { Roles = "Adm"}).
...
ErrosDeValidacao validaDTO(VeiculoDTO veiculoDTO) {
    ...
    if (string.IsNullOrEmpty(veiculoDTO.Marca)) {
        validacao.Mensagens.Add("A marca do veiculo nao pode ficar em branco.");
    }
    if (veiculoDTO.Ano < 1950) {
        validacao.Mensagens.Add("O ano do veiculo deve ser superios a 1950.");
    }
...
RequireAuthorization(new AuthorizeAtribbute { Roles = "Adm, Editor" })
.WithTags("Veiculos"); ;
